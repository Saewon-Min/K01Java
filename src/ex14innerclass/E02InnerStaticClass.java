...
내부 정적 클래스
	...
	- 이와 같은 내부 클래스를 static으로 선언하면 Nested클래스라고 한다.
	- 정적 멤버변수, 정적 멤버메소드를 가질수있다.
...
class OuterClass{
	...
	void outerMemberMethod() {
		...
		/*
		외부클래스의 멤버메소드에서는 내부정적클래스의 멤버에 접근할수있다.
	...
	static void outerStaticMethod() {
		...
		접근할수있다. (Method영역에서는 Heap영역을 참조할 수 없다.)
		 */
...
public class E02InnerStaticClass {
	...
	public static void main(String[] args) {
		...
		/*
		내부 정적 클래스의 정적멤버에 접근할때는 외부 클래스의 객체 생성없이
		클래스명으로 바로 접근가능하다.
		 */
		...
		/* 
		내부정적클래스의 인스턴스형 멤버에 접근할때는 객체를 생성한 후
		참조변수를 통해 접근해야한다.
		*/
