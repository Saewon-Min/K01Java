...
오버로딩 : 함수명은 같으나 매개변수의 개수, 타입, 순서가
	다른 경우를 말한다. 즉, 서로 다른 함수이므로 상속 받은
	하위클래스에서는 오버로딩 된 모든 메소드가 포함된다.
	
오버라이딩 : 함수명, 매개변수, 반환형까지 모두 동일한 함수를
	상속관계에서 있는 하위클래스에서 재정의 한 것을 말한다.
...
class C extends B{
	...
	상위클래스에서 정의된 반환형, 메소드명, 매개변수의 개수가
	모두 동일하다.
	 */
	@Override
	public void rideMethod() {
	...
	모두 가려지게 되어 C클래스에 정의된 메소드 하나만 남게된다.
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		// 항상 객체를 B타입으로 간주하고 참조한다.
		ref2.loadMethod();
		ref2.loadMethod(1); // => B클래스까지만 접근 가능하다.
		//ref2.loadMethod(1.1); => C클래스 영억은 접근불가
		...
		double b = 20.0; 가능
		double c = 11; 가능
		int d = (int)20.3; 불가능 -> int형으로 강제형변환
		 */
