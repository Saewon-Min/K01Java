...
public class E03CharBooleanType {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 String => "문자열" 
		 */
		char ch1 = '가'; //문자(한글자) => single quotation
		// char chStr = '가나다라';  => 에러발생] 문자열이므로 char로 선언 불가함.
		String chStr = "가나다라"; // double quotation
		...
		char ch3 = (char)(ch2+num1); // 계산결과의 형변환
		System.out.println("ch3(문자출력) => "+ch3);
		System.out.println("ch3(아스키코드출력) => "+(int)ch3);
		
		char ch4 = '1';   // 싱글쿼테이션으로 감쌌으므로 숫자1이 아닌 문자1임.
		char ch4_1 = '1' + 1; // 49 + 1 = 50
		...
		/*
		 &&(엠퍼센트) : 논리And 연산으로 양쪽 모두 참일때 참을 반환한다.
		  				나머지는 거짓을 반환한다.
		 ||(파이프) : 논리Or 연산으로 둘 중 하나만 참이면 참을 반환한다.
		 				둘다 거짓일때만 거짓을 반환한다.
		 */
		...
		bn3 = '가' > 30000; // 가: 유니코드44032 > 숫자30000
		System.out.println("bn3 = "+ bn3); // true 반환
