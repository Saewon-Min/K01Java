...
public class E07TypeCasting {
	...
	public static void main(String[] args) {
		...
		/*
		아래 코드는 자동형변환 아님.
		CPU는 int보다 작은 자료형을 int로 간주하고 연산을 진행한 후
		결과도 int로 반환한다.
		CPU의 특성이라 생각하자.
		 */
		int num1 = b1 + s1;
		 ...
		 */
		// char ch1 = b1; => 에러발생. 
		char ch2 = (char)b1; // => 명시적 형변환
		System.out.println("b1= "+b1+", ch2 = "+ch2);
		
		/*
		명시적 형변환 : 큰 상자의 자료를 작은 상자의 자료형에 할당해야 할 경우
		...
		/*
		서식문자 사용시 %.2f => 정수부는 모두 표현, 소수점이하 2자리까지만 표현
		 */
		
		
		/*
		char와 int를 연산하면 결과는 int형이므로 char형 변수에 결과를
		대입하려면 반드시 명시적(강제) 형변환 후 진행해야한다.
		*/
		char ch3 = 'A', ch4;
		int num4 = 2;
		/*
		 ch4 = ch3 + num4; 에러발생. 타입이 맞지 않아 에러발생됨. 
		 -> char < int : int로 형변환이 됨
		*/
		ch4 = (char)(ch3 + num4); // 문자A(65)에 2를 더하여 C(67)가 됨
