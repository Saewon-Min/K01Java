...
class Car{
	...
	String carModel; // 모델명
	int carYear; // 연식
	Human owner; /*   owner는 human타입이라고 말한다.
	 		소유주 : Human 클래스를 기반으로 하는 멤버변수로
	 			초기화를 위해 이름, 나이, 에너지 세가지의 값이 
	 			필요하다. 이와같이 클래스를 구성할때 또 다름
	 			객체를 멤버변수로 선언할 수 있다.
	*/
	// [멤버메소드]
	void drive() {
		System.out.println(owner.name + "이(가) "+carModel +"을 운전하다.");
		/*
		owner는 객체이므로 이름을 출력하기 위해서는 참조변수.name 으로
		작성해야 한다.
		 */
	...
	초기화 된다는 한계를 가지고 있다. 따라서 이와같은 형태로는
	만들지 않는것이 좋다.
	 */
	void initialize() {
		carModel = "람보르기니";
	...
	void initialize(String model, int year, String name,
		...
		carYear = year;
		/*
		또 다른 객체라 멤버변수로 정의되었으므로, 객체를 생성한 후
		각각의 멤버변수를 초기화해야한다.
		 */
		owner = new Human();
		owner.name = name;
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		// car1.drive();
		/*
		실행시 예외 발생됨. 자동차 소유주를 표현하는 Human 객체가
		초기화 되지 않는 상태에서 출력을 시도하여 런타임에러가 발생된다.
		 */
		...
		/*
		만약 2개 이상의 객체를 생성해야 한다면 아래의 초기화 코드가
		중복사용되어야 하므로 이와같은 방법은 사용하지 않는것이 좋다.
		 */
		Car car2 = new Car();
