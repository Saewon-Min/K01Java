...
이런 경우 반드시 자식 예외클래스부터 catch블럭을 사용해야한다.
Exception클래스는 모든 예외를 catch하기 때문에 반드시
마지막 블럭에 사용해야한다.

특정클래스의 상속구조를 보고 싶을때는 선택 후 [ Ctrl + t ] 로 확인할수있다.
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("배열에 저장된 두 숫자나누기 : "+
					array[0]/array[1]); // array[1] 자리에 0을 입력해볼것
			System.out.println("나이를 입력하세요 : ");
			int age = scanner.nextInt(); // 나이 입력시 문자를 입력해볼것
			...
			System.out.println("숫자형대의 문자만 입력가능합니다.");
		} catch (ArrayIndexOutOfBoundsException e) {
			System.out.println("배열의 인덱스를 초과하였습니다.");
		/*
		항상 Exception은 맨 마지막에	
		 */
		} catch (Exception e) {    						
