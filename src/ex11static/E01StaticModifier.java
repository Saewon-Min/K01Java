...
/*
static variable(정적변수,정적메소드)
- 멤버변수 앞에 static 지정자가 붙으면 정적 변수가 된다.
- 정적변수는 JVM(자바가상머신)에 의해 프로그램이 시작되는
	시점에 초기화된다.
- main() 메소드가 실행되기전에 이미 초기화 되므로 main() 메소드 실행시
	별도의 선언없이 변수를 사용할 수 있다.(main()메소드 실행 전에 이미 메모리상에 로드가 되어버림)
...
class MyStatic{
	...
	정적 멤버메소드] 정적메소드 안에서는 인스턴스 멤버를 사용할 수 없다.
		정적 멤버만 접근이 가능하다.
		(※main메소드에서는 static으로 선언된 메소드만 호출할 수 있다.)
	 */
	static void staticMethod() {
		//System.out.println("instanveVar = "+ instanceVar); // 오류발생 
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		인스턴스형 멤버변수는 객체별로 각각 존재하므로 아래 초기화된 갑슨
		...
		/*
		정적 멤버변수는 각각의 객체마다 멤버변수가 존재하지 않고, 메소드 영역에 하나만 생성되어
		공유되므로 아래 대입문장은 하나의 변수에 값을 할당하게 되는것이다. 따라서 결과는 
		마지막에 입력한 900이 출력된다.
		 */
		...
		myStatic.staticMethod();// 권장사항이 아니므로 클래스명을 호출하는 것이 좋다.
