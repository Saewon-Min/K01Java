...
HashMap<K,V>
	...
	- 키 값은 중복될 수 없다. 중복될 경우 덮어쓰기로 처리된다.
...
public class Ex05HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		중복 저장
			: 기존에 입력된 name키 값이 있으므로 "홍길동"이 출력된다.
			그리고 기존의 값이 "최길동"으로 갱신된다.
		 */
		
		System.out.println("name이라는 키값으로 저장된 이전의 값 : "+
					map.put("name", "최길동")); // 덮어쓰기 처리됨
		System.out.println("키값으로 중복 저장 후 객체 수 : "+map.size()); // 4개
		
		/*
		출력하기
			1. 키 값을 알고있을때 : get(키값)으로 출력한다.
		 */
		...
		/*
		삭제
			: key를 통해 삭제하고, 삭제되면 해당 value가 반환된다.
		 */
