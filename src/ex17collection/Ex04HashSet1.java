...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		3] 객체 저장 : add()메소드를 통해 저장되고, 문제가 없다면
			true가 반환된다.
		*/
		System.out.println(set.add(strObject1));
		set.add(strObject2);
		set.add(dateObject);
		set.add(number); // <- Integer객체로 Boxing처리 후 저장됨
		set.add(teacher);
		
		/*
		4] 저장된 객체수 얻기 : 5개
		*/
		System.out.println("[중복저장 전 객체수] : "+set.size());
		
		/*
		5-1] 기본클래스형의 객체 중복 저장
		 	: 기본 클래스인 경우 별도의 오버라이딩 처리없이도
		 	중복이 제거된다. 따라서 아래의 경우 false가 반환된다.
		*/
		System.out.println(set.add(strObject2)?"저장성공":"저장실패");
		System.out.println("[중복저장 후 객체수] : "+set.size());
		...
		*/
		...
		System.out.println("[중복(teacher2)저장 후 객체수] : "+
				set.size()); // 6개
		...
		*/
		...
		8] 삭제
		*/
		System.out.println(set.contains(strObject2)?
				"strObject2 있다":"strObject2 없다");
