...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/*
		만약 아래와 같이 타입매개변수 부분을 생략하거나 Object로 설정하면
		...
		add(객체)
			: 순차적으로 저장하면서 인덱스는 0부터 자동으로 부여한다
		
		add(인덱스, 객체)
			: 인덱스를 직접 부여하여 객체를 저장한다.
			단, 인덱스를 건너뛰면 예외가 발생한다.
		
		size() 
			: 컬렉션에 저장된 객체의 갯수를 반환한다.
		...
		/*
		1-3] 리스트 컬렉션 출력하기(일반 for문, 확장 for문)
		...
		1-5] 끼워넣기(삽입하기)
			add(삽입할 인덱스, 객체)
				: 해당 인덱스에 삽입하고 기존데이터는 자동으로 뒤로 밀리게 된다.
		
		 */
		System.out.println("\n==============\n");
		list.add(4, "블랙핑크_끼워넣기");
		for(Object obj : list) {
			System.out.print(obj + " ");
		}
		...
		1-6] 포함여부 확인
			contains(객체)
				: 찾고자 하는 객체가 있는경우 true를 반환한다.
				위치한 인덱스까지는 확인할 수 없다. -> 검색은 불가능(어디에 있는지 찾는것 불가능)
		 */
		...
		방법1 : 인덱스로 삭제
			remove(인덱스)
				...
				삭제 후에는 인덱스가 자동으로 재부여된다. 삭제 성공시 해당 객체를
				반환하므로 어떤 객체가 삭제되었는지 확인이 가능하다.
		 */
