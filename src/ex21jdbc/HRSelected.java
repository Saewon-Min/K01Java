	...
	public static void main(String[] args) {
		...
		try {
			...
			if (con!=null) {
				...
				/*
				1. 쿼리문 작성
					: 쿼리문을 작성할 때 주의할점은 줄바꿈을 할 때 앞뒤로
					스페이스를 삽입하는 것이다. 그렇지 않으면 서로 문장이
				...
				*/
				...
				2. 쿼리문 전송을 위한 Statement인터페이스를 통한 객체를
					생성한다. 주로 정적 쿼리문을 실행할 때 사용한다.
				 */
				Statement stmt = con.createStatement();
				
				/*
				3. 쿼리문을 전송한 뒤 실행된 결과를 ResultSet객체를 통해
					반환받는다. 해당 객체는 select쿼리를 실행했을때만 사용한다.
				 */
				ResultSet rs = stmt.executeQuery(sql);
				
				/*
				4. 반환된 결과를 갯수만큼 반환한다. next() 메소드는
					출력할 레코드가 있는지 확인할 때 사용한다. 만약
					더이상 출력할 레코드가 없다면 false를 반환한다.
				 */
				while (rs.next()) {
					/*
					...
					사용할 수 있다.
					 */
					String emp_id = rs.getString(1); // 인덱스 1번째 컬럼
				...
				/*
				5. 자원 반납 : 모든 작업을 마친 후에는 메모리 절약을 위해
					연결했던 자원을 반납한다.
				 */
				rs.close();
				stmt.close();
